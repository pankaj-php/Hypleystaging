...
class RestoreFilesService
{
    ...
    private function restoreBasePlugin($oldPath, $newPath)
    {
        ...
        // Move backup plugins && Get current plugins list (only first level dir names)
    ...
    private function disablePlugins($path)
    {
        $iterator = (new Finder())
            ->in($path)
            ->depth(0)
            /* @see \WPStaging\Pro\Backup\Service\RestoreFilesService::restoreBasePlugin For untrailingslashit notes. */
            ->notPath(untrailingslashit(WPSTG_PLUGIN_DIR))
            ->notName(self::OPTIMIZER_MU_PLUGIN_FILE)
            ->directories()
        ;
