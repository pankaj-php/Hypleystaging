				...
				<?php foreach ($field['value'] as $m_key => $menu) { ?>
					...
					if(isset($menu['menu_elements'])) {
							...
							<tr class="pricing-list-item <?php if( $z === 0) { echo 'pattern'; } ?>" data-iterator="<?php echo esc_attr($z); ?>">
								<td>
									...
									<div class="fm-input pricing-price">
										<input type="number" step="0.01" name="<?php echo esc_attr($key); ?>[<?php echo esc_attr($i); ?>][menu_elements][<?php echo esc_attr($z); ?>][price]" value="<?php echo $menu_el['price']; ?>" placeholder="<?php esc_html_e('Price','listeo_core'); ?>" data-unit="<?php echo esc_attr(get_option( 'listeo_currency')) ?>" />
									</div>
						...
						<div class="fm-input pricing-bookable-options">
										<select style="display: none;" class="chosen-select" name="<?php echo esc_attr($key); ?>[<?php echo esc_attr($i); ?>][menu_elements][<?php echo esc_attr($z); ?>][bookable_options]" id="">
		...
		<table id="pricing-list-container">
			...
			<tr class="pricing-list-item pattern" data-iterator="0">
				<td>
					...
					<div class="fm-input pricing-price"><input type="number" step="0.01" name="_menu[0][menu_elements][0][price]" placeholder="<?php esc_html_e('Price','listeo_core'); ?>" data-unit="<?php echo esc_attr(get_option( 'listeo_currency')) ?>" /></div>
					<div style="display: none;" class="fm-input pricing-bookable"><div class="switcher-tip" data-tip-content="<?php esc_html_e( 'Click to make this item bookable in booking widget','listeo_core' ); ?>"><input checked type="checkbox" class="input-checkbox switch_1" name="_menu[0][menu_elements][0][bookable]" /></div></div>
						<select style="display: none;" name="<?php echo esc_attr($key); ?>[0][menu_elements][0][bookable_options]" id="">
