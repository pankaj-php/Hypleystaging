					...
					if(isset($menu['menu_elements'])) {
						foreach ($menu['menu_elements'] as $el_key => $menu_el) { ?>
							<tr class="pricing-list-item <?php if( $z === 0) { echo 'pattern'; } ?>" data-iterator="<?php echo esc_attr($z); ?>">
								<td>
									...
									<div class="fm-input pricing-price 5555">
										<input type="number" step="1" min="1" name="<?php echo esc_attr($key); ?>[<?php echo esc_attr($i); ?>][menu_elements][<?php echo esc_attr($z); ?>][price]" value="<?php echo $menu_el['price']; ?>" placeholder="<?php esc_html_e('Price','listeo_core'); ?>" data-unit="<?php echo esc_attr(get_option( 'listeo_currency')) ?>" />
									</div>
									<div style="display: none;" class="fm-input pricing-bookable" >
										<div class="switcher-tip" data-tip-content="<?php esc_html_e( 'Click to make this item bookable in booking widget','listeo_core' ); ?>"><input checked type="checkbox"  class="input-checkbox switch_1" name="<?php echo esc_attr($key); ?>[<?php echo esc_attr($i); ?>][menu_elements][<?php echo esc_attr($z); ?>][bookable]" 
						...
						<div style="display: none;" class="fm-input pricing-bookable-options">
										...
										<div class="checkboxes in-row pricing-quanity-buttons">
											<input type="checkbox" checked class="input-checkbox" name="<?php echo esc_attr($key); ?>[<?php echo esc_attr($i); ?>][menu_elements][<?php echo esc_attr($z); ?>][bookable_quantity]" id="<?php echo esc_attr($key); ?>[<?php echo esc_attr($i); ?>][menu_elements][<?php echo esc_attr($z); ?>][bookable_quantity]" 
		...
		<table id="pricing-list-container">
			...
			<tr class="pricing-list-item pattern" data-iterator="0">
				<td>
					...
					<div class="fm-input pricing-price 4545"><input type="number" step="1" min="1" name="_menu[0][menu_elements][0][price]" placeholder="<?php esc_html_e('Price','listeo_core'); ?>" data-unit="<?php echo esc_attr(get_option( 'listeo_currency')) ?>" /></div>
					<div style="display: none;" class="fm-input pricing-bookable"><div class="switcher-tip" data-tip-content="<?php esc_html_e( 'Click to make this item bookable in booking widget','listeo_core' ); ?>"><input checked type="checkbox" class="input-checkbox switch_1" name="_menu[0][menu_elements][0][bookable]" /></div></div>
					<div style="display: none;" class="fm-input pricing-bookable-options">
